...
int main()
{
	...
	for (int i = 0; i < nm; i++)
		for (int j = 0; j < nf; j++)
		{
			switch (mcodes[i])
			{
			case 0: {start_sa(fcodes[j], n, launches, max_call_f); break; }					//метод имитации отжига (SA)
			case 1: {start_ga(fcodes[j], n, launches, psize, max_call_f); break; }			//генетический алгоритм (GA)
			case 2: {start_difevo(fcodes[j], n, launches, psize, max_call_f); break; }		//метод дифференциальной эволюции (DE)
			case 3: {start_pso(fcodes[j], n, launches, psize, max_call_f); break; }			//метод роя частиц (PSO)
			case 4: {start_gapso(fcodes[j], n, launches, psize, max_call_f); break; }		//гибрид GA и PSO
			...
			case 7: {start_cgm(fcodes[j], n, launches, max_call_f); break; }				//метод сопряженных градиентов
			}
		}
	return 0;
}
